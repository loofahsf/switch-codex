...
if (action === 'dev') {
  ...
  const binary = process.platform === 'darwin' ? 'bin/dev/Switch Codex.app/Contents/MacOS/switch-codex' : 'bin/dev/switch-codex.exe';
  run(resolve(binary), []);
} else {
  ...
  if (action === 'package') {
    ...
    const target = resolve(`release/Switch-Codex-${pkg.version}-${platform}-${arch}${app ? '.dmg' : '-setup.exe'}`);
    if (app) {

...
在侧边栏 **设置** 页面开启“每日定时调用”，选择电脑本地时间（`HH:mm`）并保存。默认关闭。应用会每天通过本机 Codex CLI，对已保存账号依次调用 `gpt-5.6-luna`，固定提示词为 `What model are you?`。

- 后台每分钟检查一次。整批任务允许最多 5 分钟的启动延迟，超过则跳过当天。
- 开始时一次性获取账号及认证快照，按账号列表顺序串行执行，并发数始终为 1。五分钟限制只针对整批启动，后面的账号排队超过五分钟仍会执行。
- 每个账号调用最多等待 120 秒，失败或超时会继续下一个账号，不自动重试。页面显示最近一批账号的等待中、执行中、成功、失败或中断状态。
...
需要安装支持 `codex exec`、`--ignore-user-config`、`--ephemeral`、`--json` 等参数的 Codex CLI。应用自动从 PATH 和常见安装目录查找，也可在设置中填写可执行文件绝对路径；Windows 指定 `codex.exe`，不使用 `.cmd` 脚本。启用时会检查 CLI 参数兼容性，不会自动安装或登录。

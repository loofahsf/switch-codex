...
- 应用驻留期间每 30 秒只检查一次 `~/.codex/auth.json`，自动保存 Codex 刷新的 Token，并在复合身份唯一匹配时安全跟随已保存账号。
- 支持通过应用窗口、macOS 应用菜单或 macOS 菜单栏状态栏图标（Status Item）快捷切换账号。
- 校验导入的凭证文件，并在切换时保留前一次激活文件的备份 `~/.codex/auth.json.switch-codex.bak`。
- 支持从设置页将全部账号导出为密码加密的 `.scbackup` 文件，并在另一台电脑的空账号库中恢复。
- 使用登录方式、用户标识和工作空间标识组成复合身份；同一 Team 下的不同用户不会仅因 `account_id` 相同而互相覆盖。未知、缺少身份或重复匹配的认证只提示处理，不自动写入。
...
设置页可以将全部已保存账号导出为一个 `.scbackup` 加密文件。备份包含账号名称、创建与更新时间以及完整认证凭据，不包含 CLI 路径、定时调用设置、运行历史、配额缓存、本地 Session 用量或源电脑文件路径。

导出密码至少需要 8 个字符，应用无法找回密码。备份文件包含可用于登录的敏感凭据，即使已经加密，也应存放在可信位置且不要分享给他人。若凭据已被服务端撤销或过期，恢复后仍需通过 Codex 重新登录或刷新。

为避免意外覆盖，账号备份只能导入到没有已保存账号的目标库。导入不会修改目标电脑现有的 `~/.codex/auth.json`，也不会恢复源电脑的当前激活账号；导入成功后会关闭认证文件自动同步。请前往账号管理手动选择账号，并按需重新启用自动同步。
...
设置页中的“认证文件自动同步”默认开启。该功能不监听或扫描 `~/.codex/` 目录，只低频读取明确的 `~/.codex/auth.json` 路径；应用启动、系统唤醒、窗口重新打开或点击“立即检查”时也会执行一次。普通 Token 刷新静默保存；检测到另一个唯一匹配的已保存账号时自动跟随；检测到新账号时可直接命名添加。完全退出应用后停止同步。Codex 还支持 `file`、`keyring`、`auto` 和 `ephemeral` 凭据存储模式；按 [OpenAI 身份验证文档](https://learn.chatgpt.com/zh-Hans/docs/auth) 配置为系统凭据存储且没有 `auth.json` 时，应用仅显示“未检测到文件”，不会尝试读取钥匙串。

在侧边栏 **设置** 页面开启“每日定时调用”，通过 24 小时制 TimePicker 选择电脑本地时间（`HH:mm:ss`）并保存，已有 `HH:mm` 设置会自动补为 `HH:mm:00`。默认关闭。应用会每天通过本机 Codex CLI，使用 `gpt-5.6-luna` 为每个已保存账号随机选择一个内置短任务，并在设定时间后随机延迟 60–300 秒独立执行。

- 后台仍每分钟检查一次，在设定的时分秒之后首次轮询时启动，并非精确到秒触发。整批任务允许最多 5 分钟的启动延迟，超过则跳过当天。
- 开始时一次性获取账号及认证快照，并固定每个账号的随机任务与计划启动时间。任务池在一轮用完前不重复，账号更多时重新打乱下一轮；延迟相对于每日设定时间计算，包含 60 秒和 300 秒。
...
任务使用独立临时 `CODEX_HOME`，不会切换账号。CLI 自动刷新的认证信息会在复合身份匹配且原凭证未被修改时原子回写至对应账号文件；若刷新的是当前账号，并且全局认证也仍是任务开始时的版本，则同时更新 `~/.codex/auth.json`，避免刷新 Token 被旧文件覆盖。临时目录完成后清理，异常退出残留会在下次启动清理。凭证不返回前端、不写日志。由于独立调用使用 `--ephemeral`，不会增加本地 Codex Session 历史；服务端仍正常计入订阅额度。

一次调用的成功仅代表 CLI 返回成功完成事件，不能保证重置已开启的五小时窗口。窗口起止时间由服务端决定，任务完成后会刷新额度数据。

账号管理页也支持手动预热：单账号按钮会立刻发送一个随机小任务；“全部预热”会为每个账号随机安排 60–300 秒延迟后并行发送小任务。手动预热使用独立的执行器和临时认证目录，不会等待、取消、改变或覆盖每日定时调用的队列、状态和最近一次执行记录；两种手动操作也可与定时调用同时进行。
...
设置和最近一次执行结果存储在账号数据目录的 `settings.json`，定时和手动预热分别使用 `scheduled-runtime/`、`manual-runtime/` 临时认证目录；均跟随 `CODEX_SWITCH_DATA_DIR`。开发模式忽略账号数据、价格缓存、临时凭证和构建产物，避免触发 Wails 重启。

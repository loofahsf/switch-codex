...
const commands: Record<string, (args: Args) => PromiseLike<unknown>> = {
  ...
  choose_accounts_backup: () => AppService.ChooseAccountsBackup(),
  export_accounts_backup: (args) => AppService.ExportAccountsBackup(args.passphrase as string),
  import_accounts_backup: (args) => AppService.ImportAccountsBackup(args.path as string, args.passphrase as string),
